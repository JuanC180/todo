...
const Registrar = () => {
  ...
  const handleSubmit = async e => {
    ...
    if ([nombre, apellido, email, password, repetirPassword, estado, rol].includes('')) {
    ...
    //   const url = `${import.meta.env.VITE_BACKEND_URL}/api/usuarios`
      const url = `/usuarios`
      const {data} = await clienteAxios.post(url, {nombre, apellido, email, password, estado, rol})
      console.log(data)
      setAlerta({
